...
import { IDepartment, IProgramOutcome, PaginationMeta } from '@/types/api';
...
export default function DepartmentsPage() {
  ...
  const fetchDepartments = async () => {
    setLoading(true);
    try {
      const response = await apiGet<IDepartment[]>('/departments', {
        page,
        limit: 10,
  ...
  const openEditModal = (dept: IDepartment) => {
    ...
    setCode(dept.code);
    setName(dept.name);
    setHasPrograms(dept.hasPrograms);
    setProgramOutcomes(dept.programOutcomes || []);
    setFormError(null);
    setIsFormOpen(true);
  };

  const openDeleteConfirm = (dept: IDepartment) => {
    setSelectedDept(dept);
  ...
  const handleSubmit = async () => {
    ...
    setFormError(null);
    try {
      // Basic validation matching backend Zod
      ...
      const payload = {
        ...
        name: name.trim(),
        hasPrograms,
        programOutcomes: hasPrograms ? [] : programOutcomes,
      };

      if (selectedDept) {
        await apiPut(`/departments/${selectedDept._id}`, payload);
  ...
  const handleDelete = async () => {
    ...
    setSubmitLoading(true);
    try {
      await apiDelete(`/departments/${selectedDept._id}`);
  ...
  const columns: ColumnConfig<IDepartment>[] = [
    ...
    { key: 'name', label: 'Name' },
    {
      ...
      label: 'Has Programs',
      render: (item) => (
        <span className={`px-1.5 py-0.5 border text-[10px] font-mono font-medium ${
          item.hasPrograms 
            ? 'border-zinc-900 bg-zinc-900 text-white dark:border-zinc-100 dark:bg-zinc-100 dark:text-zinc-900' 
            : 'border-zinc-300 dark:border-zinc-700 text-zinc-400'
        }`}>
          {item.hasPrograms ? 'YES' : 'NO'}
        </span>
      )
    },
    {
    ...
    <DashboardLayout title="Departments">
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <FormField label="Department Code" hint="e.g. CSE, EEE (2-10 chars)">
            <input
              type="text"
              required
              value={code}
          ...
          <FormField label="Department Name" hint="e.g. Computer Science & Engineering">
            <input
              type="text"
              required
              value={name}
        ...
        <div className="flex items-center space-x-2 pt-2">
          <input
            ...
            onChange={(e) => {
              ...
              setHasPrograms(checked);
              if (checked) {
                setProgramOutcomes([]);
              }
            }}
            className="h-3.5 w-3.5 border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-50 focus:outline-none cursor-pointer"
          />
          <label htmlFor="hasPrograms" className="text-xs font-mono font-medium text-zinc-700 dark:text-zinc-300 cursor-pointer">
            THIS DEPARTMENT VARY BY PROGRAM (HAS MULTIPLE ACADEMIC PROGRAMS)
          </label>
        </div>
        ...
        {!hasPrograms ? (
          ...
          <div className="pt-4 border-t border-zinc-200 dark:border-zinc-800 text-xs font-mono text-zinc-400 uppercase italic">
            Program Outcomes will be managed at the individual program level.
          </div>
        )}
      </FormModal>
      ...
      <ConfirmDialog
        ...
        message={`Are you sure you want to delete department "${selectedDept?.name || ''}" (${selectedDept?.code || ''})? This will also affect any associated programs and courses.`}
        loading={submitLoading}
      />
    </DashboardLayout>
  );
