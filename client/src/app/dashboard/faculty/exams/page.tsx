...
import { IExam, ICourseOffering, IQuestion, PaginationMeta } from '@/types/api';
...
export default function FacultyExamsPage() {
  ...
  const fetchExams = async () => {
    setLoading(true);
    try {
      // Fetch exams in their department
  ...
  const fetchDependencies = async () => {
    try {
      // Only fetch offerings assigned to me
      const offeringsRes = await apiGet<ICourseOffering[]>('/offerings/me', { limit: 100 });
      setOfferings(offeringsRes.data);
    } catch (err) {
      console.error('Failed to fetch assigned offerings', err);
    }
  };
  ...
  const getSelectedOfferingCOs = () => {
    ...
    if (offeringObj.course && typeof offeringObj.course === 'object') {
      return (offeringObj.course as any).courseOutcomes || [];
    }
    return [];
  };
  ...
  const getSelectedCourseType = () => {
    ...
    if (offeringObj.course && typeof offeringObj.course === 'object') {
      return (offeringObj.course as any).type || 'theory';
    }
    return 'theory';
  };
  ...
  const handleAddQuestion = () => {
    ...
    setQuestions([...questions, { number: `Q${questions.length + 1}`, text: '', marks: 5, coMapping: [{ co: defaultCo, percentage: 100 }] } as any]);
  };
    ...
    <RoleGuard allowedRoles={["faculty"]}>
      <DashboardLayout title="My Exams">
        <DataTable
          ...
          extraActions={(item) => (
            <button
              onClick={() => router.push(`/dashboard/faculty/exams/${item._id}/marks`)}
            ...
            {questions.length === 0 ? (
              ...
              <div className="space-y-3 max-h-60 overflow-y-auto pr-1">
                {questions.map((q, index) => (
                  <div key={index} className="border border-dashed border-zinc-200 dark:border-zinc-800 p-2.5 bg-zinc-50/50 dark:bg-zinc-900/30 space-y-2">
                    ...
                    <div className="w-full">
                      <label className="block text-[9px] font-mono text-zinc-400">QUESTION TEXT</label>
                      <textarea
                        value={q.text || ''}
                        ...
                        rows={3}
                        maxLength={2000}
                        className="mt-1 block w-full px-2 py-1.5 border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-50 font-mono text-xs focus:outline-none resize-y"
                        style={{ minHeight: '60px' }}
                      />
                      <div className="text-right text-[9px] font-mono text-zinc-400 mt-0.5">
                        {(q.text || '').length}/2000
                      </div>
                    </div>
                  </div>
