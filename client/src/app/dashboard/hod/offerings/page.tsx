...
export default function HODOfferingsPage() {
  ...
  return (
    <RoleGuard allowedRoles={["HOD", "hod"]}>
      <DashboardLayout title="HOD: Course Offerings">
        <DataTable
          columns={columns}
          data={data}
          loading={loading}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <FormField label="Batch">
              <select
                ...
                className="mt-1 block w-full px-3 py-2 border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 text-zinc-950 dark:text-zinc-50 font-mono text-xs focus:outline-none"
              >
                <option value="">SELECT BATCH</option>
                {batches.map((b) => (
                  <option key={b._id} value={b._id}>
