...
import { ICourse, IDepartment, IProgram, ICourseOutcome, ICoPoMapping, PaginationMeta, BloomLevel } from '@/types/api';
...
export default function CoursesPage() {
  ...
  // Resolves POs based on selected Department / Program
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Validation check: PO/PSO format and descriptions
      ...
      <DataTable
        ...
        extraActions={(item) => (
          <button
            ...
            className="px-2 py-1 border border-zinc-200 dark:border-zinc-800 text-[10px] font-mono uppercase tracking-tight text-zinc-600 dark:text-zinc-400 hover:border-zinc-950 dark:hover:border-zinc-200 cursor-pointer"
          >
            Matrix
          </button>
        )}
        ...
        <div className="pt-6 border-t border-zinc-200 dark:border-zinc-800 space-y-4">
          ...
          {coPoMapping.length === 0 ? (
            <p className="text-[10px] font-mono text-zinc-400 dark:text-zinc-500 uppercase italic">
              No mappings specified. Maps Course Outcomes to Program Outcomes.
            </p>
          ) : (
                          ...
                          {mapping.weight > 0 ? (
                            <span className={`inline-block w-6 py-0.5 text-center ${
                              mapping.weight === 3 
                                ...
                                : 'bg-zinc-100 text-zinc-800 dark:bg-zinc-800 dark:text-zinc-400 border border-zinc-300 dark:border-zinc-650'
                            }`}>
                              {mapping.weight}
                            </span>
                          ) : (
